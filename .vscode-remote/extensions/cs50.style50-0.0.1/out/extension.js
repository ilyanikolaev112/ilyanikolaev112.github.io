...
const fs = require("fs");
const jsFormatter = require("js-beautify");
...
let currentDiffText;
let lastSourceFileUri;
let lastFormattedFileUri;
let lastTitle;
async function activate(context) {
    ...
    async function runStyle50(filePath) {
        try {
            ...
            // SQL
            if (fileExt === 'sql') {
                const styleConfig = vscode.workspace.getConfiguration('Prettier-SQL');
                fs.readFile(sourceFileUri.fsPath, 'utf8', function (err, data) {
                    if (err) {
                        console.log(err);
                        vscode.window.showErrorMessage(err.message);
                    ...
                    fs.writeFile(formattedFilePath, (0, sql_formatter_1.format)(data, styleConfig), () => {
                        showDiffEditor(sourceFileUri, vscode.Uri.file(formattedFilePath), diffTitle);
                    });
                });
            }
...
async function showDiffEditor(sourceFileUri, formattedFileUri, title) {
    session_uuid = uuidv4();
    ...
    await exec(`diff ${sourceFileUri.fsPath.replace(/ /g, '\\ ')} ${formattedFileUri.fsPath}`, async (err, stdout, stderr) => {
        ...
        if (stdout) {
            ...
            currentDiffText = ((_a = vscode.window.activeTextEditor) === null || _a === void 0 ? void 0 : _a.document.getText()) || '';
...
function extractDiffBlocks(input, n) {
    ...
    vscode.window.tabGroups.all.forEach((tabGroup) => {
        tabGroup.tabs.forEach(async (tab) => {
            ...
            if (tab.label.match(/^style50\s.+\..+$/)) {
                if ((_a = vscode.window.activeTextEditor) === null || _a === void 0 ? void 0 : _a.document.fileName.startsWith("/tmp/style50/diff/diff_")) {
...
async function logEvent(eventType, sessionUuid = session_uuid) {
